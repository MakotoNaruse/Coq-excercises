...
Proof.
  intros n. induction n as[| n' IHn' ].
  - (* n = 0 *) simpl. reflexivity.
  - (* n = 0 *) simpl. rewrite <- plus_n_Sm. rewrite <- IHn'.
    reflexivity. Qed.
(** [] *)
...
(** One inconvenient aspect of our definition of [evenb n] is the
    ...
    with induction: *)
Theorem negb_double : forall b : bool,
  negb (negb b) = b.
Proof.
  intros [].
  - simpl. reflexivity.
...
Proof.
  ...
  - (* n = S n' *) rewrite -> IHn'. 
    ...
    { simpl. reflexivity. }
    assert (H2: negb(negb( evenb n')) = evenb n').
...
Proof.
  intros n. induction n as [|n' IHn' ].
  - (* n = 0 *) simpl. reflexivity.
  - (* n = S n' *)  rewrite -> IHn'. simpl. reflexivity. Qed.
(** [] *)
