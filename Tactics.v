...
(* rewriteは書き換えるのみだがapplyは十分条件を遡ってゴールを変更している *)
...
Proof.
  ...
  rewrite -> eq2. apply eq1. reflexivity. Qed.
(** [] *)
...
Example discriminate_ex3 :
  forall (X : Type) (x y z : X) (l j : list X),
    ...
    x = z.
Proof.
  intros X x y z l eq1 eq2. discriminate eq2. Qed.
(** [] *)
...
Proof.
  ...
  - intros m eq. destruct m as [| m'].
    + discriminate eq.
    + inversion eq. apply f_equal. apply IHl'. apply H0.
Qed.
(** [] *)
...
(**
証明: nについての帰納法で「任意のnについて n =? m がtrueならば n = m」を示す。
・n=0の場合: 「任意のmについて 0 =? m がtrueならば 0 = m」を示す。
    m=0の時は自明
    m=S(m')の場合は前提 0 =? S m' = trueが不成立
・n=S n'の場合、
  ただし「任意のnについて n' =? m がtrueならば n' = m」であるとする。
  この時、「任意のmについて S n' =? m がtrueならば S n' = m」を示す。
    m = 0の場合、前提が不成立
    m = S m'の場合
    S n' =? S m' = trueとする
    これををinversionするとn' =? m' = trueとなる。
    帰納法の仮定より、mとしてm'をとるとS n' = S m'となる。
帰納法により題意は示された。
 *)
...
(** Give an informal proof of this lemma that corresponds to your
   ...
   Proof: 
証明: nについての帰納法で「任意のnについて (n =? m) = (m =? n) 」を示す。
・n=0の場合: 「任意のmについて (0 ?= m) = (m =? 0) 」を示す。
    m=0の時は自明
    m=S(m')の場合は false = false で成立
・n=S n'の場合、
  ただし「任意のnについて (n' =? m) = (m =? n')」であるとする。
  この時、「任意のmについて (S n' ?= m) = (m =? S n')」を示す。
    m = 0の場合、前提が不成立
    m = S m'の場合
    (S n' =? S m') = (S m' =? S n') となるが、
    これは(n' =? m) = (m' =? n')と同値である。
    帰納法の仮定より、題意は示される。*)
(** [] *)
...
Proof.
  ...
  - unfold existsb'. simpl. destruct (test n).
    + simpl. reflexivity.
